...
class Router{
    ...
    */
    function getMainController (){
        ...
        // Если у меня есть динамический маршрут
        $url = explode("/", $url);
    ...
    */
	...
	public static function BuildItemButton($data, $parent_id) {
		$res = '<aside class="col-3 menu">';
		foreach ($data as $key => $item) {
				$res .= '<a role="button" class="'.$item['class'] .'" href="' . $item['slug'] . '" title="' . $item['slug'] . '" >' . $item['text'] . "</a>" ;
		}
		$res.= "</aside>";
		return $res;
	}
