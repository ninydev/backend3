 ...
 */
 ...
 jQuery.fn.liValidForm = function(options){
	...
	return this.each(function(){
		...
		var but = $('[type=submit]',form);
		var mand_el = valid_form.length; //Кол-во обязательных эл-тов
		var psevdo_but = $('<div>').css({width:but.outerWidth(),height:but.outerHeight(),left:but.position().left,top:but.position().top,position:'absolute',zIndex:'100',marginLeft:but.css('marginLeft'),marginTop:but.css('marginTop'),opacity:'0.1'}).addClass('psevdo_but')
		...
		valid_form.not(o.noValid).each(function(){
			...
			if(el_val != '' && el_val != el_dataVal ){
				if(el_form.is(':not(".email")') && el_form.is(':not(".url")') && el_form.is(':not(".digits")') && el_form.is(':not("[name=hislo]")') && el_form.is(':not("[type=radio]")') && el_form.is(':not("[type=checkbox]")')){
					remove_w(el_form);
				}else{
					//if email
					if(el_form.is('.email')){
					...
					if(el_form.is('[type=radio]') || el_form.is('[type=checkbox]')){
						if(ValidRadio(el_form)){
							$('[name='+el_form.attr('name')+']').each(function(){$(this).removeClass('wrong');})
						}else {
							$('[name='+el_form.attr('name')+']').each(function(){$(this).addClass('wrong');})
						}	
					}
				}
			}else {
		...
		function valider(z_el_form,z_el_val){
			...
			if(z_el_val != '' && z_el_val != z_el_form.data('val')){
				...
				if(z_el_form.is(':not(".email")') && z_el_form.is(':not(".url")') && z_el_form.is(':not(".digits")') && z_el_form.is(':not("[name=hislo]")') && z_el_form.is(':not("[type=radio]")') && z_el_form.is(':not("[type=checkbox]")')){
					remove_w(z_el_form);
					remove_c(z_el_form);
				}else{
					//if email
					...
					if(z_el_form.is('[type=radio]') || z_el_form.is('[type=checkbox]')){
						if(ValidRadio(z_el_form)){
							$('[name='+z_el_form.attr('name')+']').each(function(){$(this).removeClass('wrong').removeClass('indicator');})
						}else {
							$('[name='+z_el_form.attr('name')+']').each(function(){$(this).addClass('wrong').addClass('indicator');})
						}	
					}
